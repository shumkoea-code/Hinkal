      ...
      <div className="container glass-nav-inner">
        ...
        >
          {headerMainPages.map((page: any) => (
            <Link key={page.id} href={publicPagePath(page.slug)} style={getLinkStyle(publicPagePath(page.slug))}>
              {page.title}
            </Link>
          ))}
          ...
          <Link href="/events" style={getLinkStyle('/events')}>
            Афиша
          </Link>
          {(siteSettings?.galleryPageEnabled ?? true) && modOn(siteSettings, 'gallery') && (
            <Link href="/gallery" style={getLinkStyle('/gallery')}>
              Галерея
            </Link>
          )}
          <Link href="/places" style={getLinkStyle('/places')}>
            Куда сходить
          </Link>
          <Link href="/news" style={getLinkStyle('/news')}>
            Новости
          </Link>
          {modOn(siteSettings, 'vacancies') && (
            <Link href="/vacancies" style={getLinkStyle('/vacancies')}>
              Вакансии
            </Link>
          )}
          {modOn(siteSettings, 'contests') && (
            <Link href="/contests" style={getLinkStyle('/contests')}>
              Конкурсы
            </Link>
          )}
