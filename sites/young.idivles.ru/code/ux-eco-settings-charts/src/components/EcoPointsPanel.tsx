...
export default function EcoPointsPanel({ compact, onBalanceChange }: Props) {
  ...
  const [shopOpen, setShopOpen] = useState(!compact);
  ...
  const post = async (body: Record<string, unknown>, okMsg: string) => {
    ...
    try {
      ...
      load();
      refresh();
      // Voice/theme apply via loadout — refresh VoiceProvider after buy/equip
      toast.success(okMsg);
    } catch (e) {
    ...
    <section className={`eco-panel${compact ? ' eco-panel--compact' : ''}`} aria-label={t('eco.title', 'Эко-баллы')}>
      ...
      {shopOpen
        ? grouped.map(([slot, items]) => (
            <details key={slot} className="eco-panel__slot-group" open={!compact || slot === 'frame'}>
              <summary className="eco-panel__slot-title">
                        ...
                        <button
                          ...
                          onClick={() =>
                            void post(
                              ...
                              item.slot === 'voice'
                                ? t('eco.toast.buyVoice', 'Куплено и включено')
                                : t('eco.toast.buy', 'Куплено и надето')
                            )
                          }
                        >
