...
export default function ProfileHeroCard({
  ...
  const [levelLabel, setLevelLabel] = useState('Ур. 1');
  ...
  const save = async (next: string[]) => {
    ...
    try {
      ...
      toast.error('Не удалось сохранить значки');
    } finally {
      setSaving(false);
    }
  };
  ...
  const ratingItems = buildRatingItems({
    ...
    item.kind === 'LEVEL'
      ? { ...item, value: levelLabel, hint: levelBlurb || item.hint }
      : item.kind === 'ECO'
