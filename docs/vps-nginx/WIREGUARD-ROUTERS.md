...
1. Админ: секретный URL из `admin.bootstrap` (`admin_url=`) → создать пир → скачать `.conf` **или** «Создать ссылку».
2. Пользователь открывает share-ссылку (без пароля админа) → скачивает конфиг / QR.
3. Импорт на роутер — разделы ниже.

Полное описание панели, 2FA, лимитов и деплоя: **[WG-PANEL.md](./WG-PANEL.md)**.
